...
exports.index = asyncHandler(async (req, res, next) => {
    ...
    if (req.query.mode === 'search') {
        users = await User.find({ $or: [ { 'username': { $regex: req.query.search }}, { 'first_name' : { $regex: req.query.search }}, { 'last_name' : { $regex: req.query.search }} ]}, undefined, { skip, limit: 1 }).populate('friends sentRequests receivedRequests').exec();
    } else {
        users = await User.find({}, undefined, { skip, limit: 2 }).populate('friends').sort({ last_name: 1 }).exec();
    }
...
exports.userFriends = asyncHandler(async (req, res, next) => {
    ...
    if (req.query.mode === 'search') {
        ...
        friends = await User.find({ friends: req.params.userId }, undefined, { skip, limit: 1 }).populate('friends').exec()
    }
