...
exports.search = asyncHandler(async (req, res, next) => {
    ...
    if (req.query.view === 'posts') {
        posts = await Post.find({ 'text': { $regex: req.params.topic }}, undefined, { skip, limit: 5 }).populate('author').populate({ path: 'comments', populate: { path: 'author' } }).sort({ 'time': -1, '_id': 1 }).exec();
